...
## Part A
```{r}
getSymbols("TSLA", src = "yahoo", from = "2014-01-01")
getSymbols("ILMN", src = "yahoo", from = "2014-01-01")
```
...
ilmnSplit <- split(ILMN[, 4], f = "months", k = 4) #Uses only closing values
ilmnModels <- lapply(ilmnSplit, getArimaModel)
table(unlist(ilmnModels))

tslaSplit <- split(TSLA[, 4], f = "months", k = 4) #Uses only closing values
...
arima(ILMN, order = c(0, 1, 0)) %>% forecast(h = 12) %>% plot

# TSLA
TSLA <- tslaSplit[[length(tslaSplit)]]
arima(TSLA, order = c(0, 1, 0)) %>% forecast(h = 12) %>% plot
```
